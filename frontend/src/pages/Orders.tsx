...
export default function Orders() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      if (isAdmin) {
        const m = await apiFetch<ApiResponse<Profile[]>>("/profiles?role=mesero")
  ...
  const createOrder = async (status: "saved" | "closed") => {
    console.log("[orders] createOrder click", { status, role: profile?.role, selected, total })
    ...
    try {
      ...
      setSelected({})
      toastOk(status === "closed" ? "Orden creada y cerrada" : "Orden creada en Saved")
      loadAll()
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : "error"
  ...
  const setOrderStatus = async (orderId: number, status: "saved" | "closed") => {
    console.log("[orders] setOrderStatus", { orderId, status, role: profile?.role })
    ...
    try {
      const r = await apiFetch<ApiResponse<Order>>(`/orders/${orderId}/status`, {
        method: "PATCH",
  ...
  const assignOrder = async (orderId: number, assignedTo: string) => {
    ...
    try {
      const r = await apiFetch<ApiResponse<Order>>(`/orders/${orderId}/assign`, {
        method: "PATCH",
      ...
      <div className="container-app py-6">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
          <div>
            ...
            <div className="text-sm text-slate-400">
              Mesero crea y se asigna solo · Admin puede reasignar · Close por mesero asignado o admin
            </div>
          </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-slate-400">Total en tiempo real · UX con botones + / -</div>
              </div>

              <div className="text-right">
                <div className="text-xs text-slate-400">Total</div>
                <div className="text-lg font-black text-emerald-200">${total.toFixed(2)}</div>
              </div>
            </div>
          </CardHeader>
          ...
          <CardBody>
            <div className="grid gap-2 sm:grid-cols-2">
              {products.map((p) => {
                ...
                return (
                  <div key={p.id} className="rounded-2xl border border-slate-800 bg-slate-950/40 p-4 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <div className="font-black truncate">{p.name}</div>
                      <div className="text-xs text-slate-400">${p.price} · line ${line.toFixed(2)}</div>
                    </div>

                    <div className="flex items-center gap-2">
                      <Button variant="danger" size="sm" onClick={() => toggleQty(p.id, -1)}>
                        -
                      </Button>
                      <div className="w-10 text-center font-black">{qty}</div>
                      <Button variant="primary" size="sm" onClick={() => toggleQty(p.id, 1)}>
                        +
                      </Button>
                    </div>
                  </div>
        ...
        <div className="mt-5 grid gap-3">
          ...
          {!loading && !orders.length ? (
            <EmptyState title="No orders" description="Creá una orden arriba para que aparezca en el dashboard" />
          ) : null}
          ...
          {!loading &&
            orders.map((o) => {
              ...
              const assignedLabel = o.assigned_to_profile?.email || o.assigned_to || "unassigned"
                ...
                <Card key={o.id}>
                  <CardHeader>
                    ...
                    <div className="mt-1 text-xs text-slate-400">
                      {formatDateTime(o.created_at)} · created by {createdLabel} · assigned to {assignedLabel}
                    </div>
                  </CardHeader>
                  ...
                  <CardBody>
                      ...
                      <div className="mb-3 grid gap-2 sm:grid-cols-[1fr_auto] sm:items-center">
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            console.log("[orders] admin select assigned_to", { orderId: o.id, v })
                    ...
                    <div className="mt-4 flex items-center justify-between gap-3">
                      <div className="text-xs text-slate-400">
                        {isClosed ? "Orden cerrada" : canTouch ? "Podés cerrar esta orden" : "No sos el mesero asignado"}
                      </div>

                      {!isClosed ? (
                        <Button
                          variant={canTouch ? "primary" : "secondary"}
                          onClick={() => canTouch && setOrderStatus(o.id, "closed")}
                          disabled={!canTouch}
                        >
                          Mark Closed
                        </Button>
