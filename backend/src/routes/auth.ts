...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    console.log("[auth][POST /register] createUser", {
      userId: created.data.user?.id,
      err: created.error?.message,
    })

    if (created.error || !created.data.user) return err(res, 500, created.error?.message || "createUser failed")
    ...
    if (ins.error) {
      ...
      return err(res, 500, ins.error.message)
    }
