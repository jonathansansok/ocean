...
ordersRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const q = await supabaseAdmin
      .from("orders")
      .select("id,status,total,created_at,created_by,assigned_to,order_items(id,product_id,qty,unit_price,line_total)")
      .order("id", { ascending: false });

    console.log("[orders][GET] orders db", { error: q.error?.message, count: q.data?.length });
    ...
    console.log("[orders][GET] profiles db", { error: prof.error?.message, count: prof.data?.length });
...
ordersRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const requestedAssignedTo = typeof req.body?.assignedTo === "string" ? req.body.assignedTo : null;
    ...
    if (!isAdmin && requestedAssignedTo && requestedAssignedTo !== req.user!.id) {
      console.log("[orders][POST] mesero tried assign other", { requestedAssignedTo, self: req.user!.id });
    ...
    const orderIns = await supabaseAdmin
      .from("orders")
      .insert({ created_by: req.user!.id, assigned_to: assignedTo, status: st.data, total })
...
ordersRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    console.log("[orders][PATCH /:id/assign] start", { orderId, body: req.body, user: req.user });
...
ordersRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    const orderId = Number(req.params.id);
    console.log("[orders][PATCH /:id/status] start", { orderId, body: req.body, user: req.user });
    ...
    if (!isAdmin && !isOwner) return err(res, 403, "You can only change status of your assigned orders");

    if (cur.data.status === "closed" && parsed.data.status !== "closed") {
      console.log("[orders][PATCH /:id/status] blocked reopen", { from: cur.data.status, to: parsed.data.status });
